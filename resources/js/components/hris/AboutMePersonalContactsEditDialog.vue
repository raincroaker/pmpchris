...
const channelOptions = ['Mobile', 'Home', 'Work'] as const;

const maxContactRows = 3;

type PersonalContactDraft = {
...
const rows = ref<PersonalContactDraft[]>([]);
const attemptedSave = ref(false);
const processing = ref(false);
const fieldErrors = ref<PageErrorsBag>({});

function cloneFromProfile(): void {
    rows.value = props.profile.personal_contacts
        .slice(0, maxContactRows)
        .map((c) => ({
...
function addRow(): void {
    if (rows.value.length >= maxContactRows) {
        return;
    }
...
function onSave(): void {
    ...
    const hasMissingNumber = nonempty.some(
        (r) => r.contact_number.trim() === '',
    );
    if (hasMissingNumber) {
        return;
    }
    ...
    const emergencyPayload = props.profile.emergency_contacts.map((c) => ({
        channel_label: c.channel_label,
    ...
    router.patch(
        ...
        },
        {
            ...
            onSuccess: () => {
                appToast.success('Contact numbers saved.');
        ...
        <DialogContent class="sm:max-w-xl">
            ...
            <ScrollArea :class="aboutMeDialogScrollAreaClass">
                <div class="grid gap-6 px-1 py-1">
                    <p
                        v-if="
                            fieldErrors.personal !== undefined &&
                    ...
                    <Button
                        ...
                        :disabled="rows.length >= maxContactRows"
                        @click="addRow"
                    >
                        <Plus class="mr-2 size-4" aria-hidden="true" />
                        {{
                            rows.length >= maxContactRows
                                ? 'Maximum 3 contacts'
                                : 'Add contact'
                        }}
                    </Button>
                    ...
                    >
                        Add at least one contact with a phone number or email.
                    </p>
                </div>
            </ScrollArea>
