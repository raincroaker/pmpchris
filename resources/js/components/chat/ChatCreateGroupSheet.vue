...
const emit = defineEmits<{
    (
        event: 'create-group',
        payload: { name: string; description: string; memberIds: number[] },
    ): void;
}>();
...
const selectedMembers = computed(() =>
    props.employees.filter((employee) =>
        selectedMemberIds.value.includes(employee.id),
    ),
);
    ...
    <Sheet v-model:open="open">
        ...
        >
            ...
            >
                ...
                <SheetDescription>
                    Set group details first, then add members before creating a
                    conversation.
                </SheetDescription>
            </SheetHeader>
            ...
            <div class="flex min-h-0 flex-1 flex-col overflow-hidden">
                <div
                    class="grid grid-cols-3 gap-2 border-b border-sidebar-border/70 px-4 py-3"
                >
                    <Button
                        type="button"
                ...
                <ScrollArea class="min-h-0 flex-1">
                    <div v-if="step === 1" class="space-y-4 px-4 py-4">
                        <div class="space-y-2">
                            ...
                            <Input
                                id="new-group-name"
                                v-model="groupName"
                                placeholder="e.g. Branch Operations Team"
                            />
                        </div>
                        <div class="space-y-2">
                            <Label for="new-group-description"
                                >Description</Label
                            >
                            <Textarea
                        ...
                        <div
                            class="rounded-md border border-dashed border-sidebar-border/70 bg-muted/20 p-3 text-xs text-muted-foreground"
                        >
                            Group photo upload can be added here once backend
                    ...
                    <div v-else class="space-y-3 px-4 py-4">
                        <div
                            class="rounded-md border border-sidebar-border/70 bg-muted/20 p-3"
                        >
                            <p
