...
class StoreCalendarEventRequest extends FormRequest
{
    public function authorize(): bool
    {
        ...
        if ($routeName === 'calendar.company-events.store') {
        ...
        if ($routeName === 'calendar.branch-events.store') {
            ...
            $branchId = (int) $this->session()->get(BranchContextService::SESSION_BRANCH_ID, 0);
            if ($branchId <= 0) {
                return false;
            }
            ...
            return in_array(
                $branchId,
                app(BranchContextService::class)->managedBranchRootIdsFor($user),
                true,
            );
        }

        if ($routeName === 'calendar.team-events.store') {
            if ($user->hasAnyRole([Role::CODE_SUPER_ADMIN, Role::CODE_HR_HEAD])) {
                return true;
            }
    ...
    {
        $value = $this->input('unit_id', $this->input('unitId'));
        if ($value === null || $value === '') {
            return null;
        }
